...
fun isNumberHappy(number: Int): Boolean {
	...
	return if ((first + second) == (third + fourth)) true else false
}
...
fun daysInMonth(month: Int, year: Int): Int = when (month) {
	1, 3, 5, 7, 8, 10, 12 -> 31
	2 -> {if((year > 1918) && (year % 4 == 0)) 29 else 28}
	else -> 30
}
...
fun brickPasses(a: Int, b: Int, c: Int, r: Int, s: Int): Boolean = when {
	((a <= r) and (b <= s)) or ((b <= r) and (c <= s)) or ((a <= r) and (c <= s)) -> true
	((b <= r) and (a <= s)) or ((c <= r) and (a <= s)) or ((c <= r) and (b <= s)) -> true
	else -> false
}